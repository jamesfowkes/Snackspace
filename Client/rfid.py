...
class CommandSet(object): #pylint: disable=R0903
...
class RFIDReader:
    """ Implements method for reading from the RFID reader via serial.
    Also implements "fake" RFID swipe for testing """
    def __init__(self, port):
        if port is not None:
            self.serial_io = Serial(port, 9600, timeout=1)
        else:
            self.serial_io = None
            self.fake_next_read = False

    def poll(self):
        """ Get any new data from the card reader """
        rfid_data = None
        
        if self.serial_io is not None:
            packet = Packet(cmd=Mifare.GetSNR, data=[0x26, 0, 0])
            reply = packet.execute(self.serial_io)
            rfid_data = reply.data
        else:
            if self.fake_next_read:
                self.fake_next_read = False
                rfid_data = [0x1B, 0x7F, 0x2D, 0x2D]
        
        return rfid_data
            
    def set_fake_rfid(self):
        """ Set a flag so that on next poll the fake RFID data is sent """
        if self.serial_io is None: # But only if the serial port is not set
            self.fake_next_read = True
                        
    @staticmethod
    def mangle_rfid(rfid):
        """ Translate RFID from card format into DB format """
        mangled = ""
